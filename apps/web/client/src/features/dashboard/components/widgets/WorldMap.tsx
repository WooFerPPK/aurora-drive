...
export default function WorldMap({ w, h }: WorldMapProps) {
  ...
  useEffect(() => {
    ...
    type ReplayRow = [number, number, number, number, [number, number, number]]
    const rows = (replayFramesData?.data ?? []) as ReplayRow[]
    const buf: TrailSample[] = []
    for (const row of rows) {
      const [tRow, speed_mps, throttle, brake, pos] = row
  ...
  useFrameLoop((frame, { frameAgeMs }) => {
    ...
    if (replayBufRef.current) {
      ...
      const tScrub = replay.tRef?.current ?? 0
      ...
      const replayFrame: OverlayFrameView = {
        motion: {
  ...
  const onCalibrationSaved = async (next: Calibration): Promise<void> => {
    setSaving(true)
    try {
      await patchSettings({ worldMap: { calibration: next } })
      setCalOpen(false)
  ...
  const onCalibrationCleared = async (): Promise<void> => {
    setSaving(true)
    try {
      await patchSettings({ worldMap: { calibration: null } })
      trailBufRef.current = []
      rebuildHeatmap([], heatmapFieldRef.current, heatmapPolysRef.current)
...
interface OverlayFrameView {
  motion?: { speed_mps?: number; orientation?: { yaw?: number } }
}

function writeOverlay(root: HTMLElement | null, frame: OverlayFrameView | null, pos: unknown, tier: 'compact' | 'standard' | 'hero'): void {
